...
ImportPayeeAsso *	last_added_assoc;
...
static ImportFormat builtin_formats[] =
{
 ...
 * \return
 **/
static void gsb_import_associations_free_assoc (ImportPayeeAsso *assoc,
												gpointer data)
{
 ...
 * \return
 **/
static gint gsb_import_add_currency (ImportAccount *compte)
{
	GtkWidget *dialog;
	GtkWidget *checkbox;
	GtkWidget *vbox;
	...
	message = g_strdup_printf (_("The account currency imported %s is %s.\nThis currency "
								 "doesn't exist so you have to create it by selecting OK.\n"
								 "\n"
								 "Do you create it?"),
							   compte->nom_de_compte,
							   compte->devise);
	hint = g_strdup_printf (_("Can't associate ISO 4217 code for currency '%s'."), compte->devise);
 ...
 **/
static gint gsb_import_create_imported_account (ImportAccount *imported_account)
{
	gint account_number;
	gint kind_account;
 ...
 * \return
 **/
static GtkWidget *gsb_import_create_page_recapitulatif (ImportAccount *compte)
{
	GtkWidget *vbox;
	GtkWidget *button;
	GtkWidget *hbox;
	...
	utils_labels_set_alignment (GTK_LABEL (label), 0, GSB_CENTER);
													 ...
													 gchar *pointeur_char)
{
	...
	if (extension)
	{
		...
		while (tmp)
		{
			ImportFormat *format = (ImportFormat *) tmp->data;
																 ...
																 gpointer model)
{
	...
	if (a_conf->force_import_directory)
	{
		...
		if (contents == NULL)
		{
			...
			basename = g_path_get_basename (nom_fichier);
											   ...
											   GtkWidget *parent)
{
	...
	format = (ImportFormat *) tmp_list->data;
 ...
 *
 * \param format		A pointer to a structure describing this import format.
 *
 * \return
 **/
static void gsb_import_register_ImportFormat (ImportFormat *format)
{
	gchar *tmp_str;
...
static GtkWidget *gsb_import_create_file_selection_page (GtkWidget *assistant)
{
	...
	{
		GtkTreeIter iter;
		ImportFormat *format = (ImportFormat *) tmp_list->data;
...
static gboolean gsb_import_enter_resume_page (GtkWidget *assistant)
{
	...
	while (files)
	{
		ImportFile *imported = files->data;
		GSList *tmp_list = ImportFormats;
 ...
 * \return void
 **/
static void gsb_import_lookup_budget (ImportTransaction *imported_transaction,
									  gint transaction_number)
{
	gint budget_number;
	gchar **tab_str;

	tab_str = g_strsplit (imported_transaction->budget, ":", 2);
 ...
 * \return
 **/
static GtkWidget *gsb_import_progress_bar_affiche (ImportAccount *imported_account)
{
	GtkWidget *assistant;
	GtkWidget *progress;
	GtkWidget *hbox;
 ...
 **/
static gint gsb_import_create_transaction (ImportTransaction *imported_transaction,
										   gint account_number,
										   gchar *origine)
{
	gchar **tab_str;
	...
	else
	{
		if (imported_transaction->categ && strlen (imported_transaction->categ))
		{
			...
			if (imported_transaction->categ[0] == '[')
			{
				...
				gsb_data_transaction_set_bank_references (transaction_number, imported_transaction->categ);
 ...
 * \return
 **/
static void gsb_import_confirmation_enregistrement_ope_import (ImportAccount *imported_account,
															   gint account_number,
															   GtkWindow *parent)
{
	GtkWidget *dialog;
	...
	else
	{
		...
		tmp_str = g_strdup (_("Some imported transactions seem to be already saved."
	...
	while (tmp_list)
	{
		...
		else if (w_etat->fusion_import_transactions && ope_import->ope_correspondante > 0)
		{
			ope_import->action = 0;
			if (ope_import->operation_ventilee)
				action_derniere_ventilation = 0;
 ...
 * return
 **/
static gboolean gsb_import_define_action (ImportAccount *imported_account,
										  gint account_number,
...
static GDate *gsb_import_get_first_date (GSList *import_list)
{
	...
	while (tmp_list)
	{
		...
		if (!first_date || g_date_compare (imported_transaction->date, first_date) < 0)
 ...
 * \return
 **/
static void gsb_import_add_imported_transactions (ImportAccount *imported_account,
												  gint account_number,
												  GtkWindow *parent)
{
	GSList *tmp_list;
 ...
 * \return
 **/
static void gsb_import_create_imported_transactions (ImportAccount *imported_account,
													 gint account_number)
{
												 ...
												 gint account_number)
{
	...
	while (tmp_list)
	{
		...
		gtk_list_store_set (store,
							...
							1, tmp_date,
							2, ope_import->tiers,
							3, utils_real_get_string_with_currency (ope_import->montant, ope_import->devise, TRUE),
							-1);
		tmp_list = tmp_list->next;
	}
	...
	g_signal_connect (renderer,
					  ...
					  store);
	column = gtk_tree_view_column_new_with_attributes (_("Mark"), renderer, "active", 0, NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste_ope_celibataires), column);

	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes (_("Date"), renderer, "text", 1, NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste_ope_celibataires), column);

	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes (_("Payee"), renderer, "text", 2, NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste_ope_celibataires), column);

	renderer = gtk_cell_renderer_text_new ();
	column = gtk_tree_view_column_new_with_attributes (_("Amount"), renderer, "text", 3, NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste_ope_celibataires), column);
 ...
 * \return
 **/
static void gsb_import_pointe_opes_importees (ImportAccount *imported_account,
											  gint account_number)
{
	GSList *list_ope_import_celibataires;
	GSList *list_ope_retenues;
			...
			&& (compte->action != IMPORT_CREATE_ACCOUNT || !strcmp (compte->origine, "CSV")))
		{
			...
			if (!strlen (name))
			{
				...
				tmp_str = g_strdup_printf (_("You want to create an import rule for the account %s "
											...
											gsb_data_account_get_name (account_number));
				name = dialogue_hint_with_entry (tmp_str,
												 _("No name for the import rule"),
												 _("Name of the rule: "));
				g_free (tmp_str);
...
void gsb_import_assistant_importer_fichier (void)
{
	...
	tmp_str = g_strconcat (_("This assistant will help you import one or several "
						   ...
						   format_str,
						   NULL);
	assistant = gsb_assistant_new (_("Importing transactions into Grisbi"), tmp_str, "gsb-import-32.png", NULL);
	g_free (format_str);
	g_free (tmp_str);
...
GSList *gsb_import_import_selected_files (GtkWidget *assistant)
{
	...
	while (valid)
	{
		...
		imported = g_malloc0 (sizeof (ImportFile));
		gtk_tree_model_get (GTK_TREE_MODEL (model),
							&iter,
							IMPORT_FILESEL_SELECTED, &selected,
...
gchar *gsb_import_unique_imported_name (const gchar *account_name)
{
	...
	do
	{
		ImportAccount *tmp_account;

		tmp_account = (ImportAccount *) tmp_list->data;
												...
												GrisbiAppConf *a_conf)
{
	...
	tmp_str = g_strdup_printf (_("Imported transactions will mark transactions in the account %s.\n"),
							   gsb_data_account_get_name (gsb_data_import_rule_get_account (rule)));
	...
	{
		tmp_str2 = g_strdup_printf (_("Amounts of the transactions will be inverted.\n"));
...
void gsb_import_by_rule (gint rule)
{
	...
	while (array[i])
	{
		...
		{
			gchar *tmp_str = g_path_get_basename (filename);
			gchar *tmp_str2 = g_strdup_printf (_("%s was not imported successfully. An error occurred "
												 "while getting the transactions."),
											   tmp_str);
			dialogue_error (tmp_str2);
			g_free (tmp_str);
			g_free (tmp_str2);
		...
		while (liste_comptes_importes)
		{
			...
			{
				account->devise = g_strdup (gsb_data_currency_get_code_iso4217 (gsb_data_import_rule_get_currency (rule)));
			}
