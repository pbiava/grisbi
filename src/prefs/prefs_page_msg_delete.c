														 ...
														 GdkEvent *ev)
{
	if (ev->button == LEFT_BUTTON)
    {
		...
		g_signal_handlers_block_by_func (tree_view, prefs_page_msg_delete_list_button_press, NULL);
		...
		g_signal_handlers_unblock_by_func (tree_view, prefs_page_msg_delete_list_button_press, NULL);
	}
    return FALSE;
}
...
/* Disable: error: format not a string literal , argument types not checked [-Werror=format-nonliteral]
  141 |                                 tmp_str = g_strdup_printf (_((warning+i)->hint), filename);
      |                                 ^~~~~~~
*/
#if defined(__GNUC__)
...
static GtkWidget *prefs_page_msg_delete_create_tree_view (PrefsPageMsgDelete *prefs)
{
    ...
    g_signal_connect (G_OBJECT (treeview),
					  "button-press-event",
					  G_CALLBACK (prefs_page_msg_delete_list_button_press),
					  NULL);
...
static void prefs_page_msg_delete_setup_page (PrefsPageMsgDelete *page)
{
	...
	gtk_box_prepend (GTK_BOX (priv->vbox_msg_delete), head_page);
...
static void prefs_page_msg_delete_class_init (PrefsPageMsgDeleteClass *klass)
{
	...
	gtk_widget_class_set_template_from_resource (GTK_WIDGET_CLASS (klass),
												 "/org/gtk/grisbi/prefs/prefs_page_msg_delete.ui");
