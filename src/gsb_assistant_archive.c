...
static void gsb_assistant_archive_add_contra_transaction_to_list (gint transaction_number)
{
	...
	if (contra_transaction_number > 0)
	{
		...
		if (!g_slist_find (list_transaction_to_archive, contra_transaction_pointer))
			list_transaction_to_archive = g_slist_append (list_transaction_to_archive, contra_transaction_pointer);

		/* if the contra-transaction is a child of split,
		 * we need to add the split and all children to the list */
		gsb_assistant_archive_add_children_to_list (gsb_data_transaction_get_mother_transaction_number
...
static void gsb_assistant_archive_add_children_to_list (gint transaction_number)
{
	...
	if (gsb_data_transaction_get_split_of_transaction (transaction_number))
	{
		...
		/* add the split */
		if (!g_slist_find (list_transaction_to_archive, gsb_data_transaction_get_pointer_of_transaction
						   (transaction_number)))
		...
		{
			/* first add the child */
			if (!g_slist_find (list_transaction_to_archive, child_list->data)
				&& gsb_data_transaction_get_transaction_number (child_list->data) > 0)
				list_transaction_to_archive = g_slist_append (list_transaction_to_archive, child_list->data);

			/* check if the child is a transfer */
			gsb_assistant_archive_add_contra_transaction_to_list (gsb_data_transaction_get_transaction_number
																  (child_list->data));
...
static void gsb_assistant_archive_add_transaction_to_list (gpointer transaction_pointer)
{
	...
	if (!g_slist_find (list_transaction_to_archive, transaction_pointer))
		list_transaction_to_archive = g_slist_append (list_transaction_to_archive, transaction_pointer);
...
static gboolean gsb_assistant_archive_update_labels (GtkWidget *assistant)
{
	...
	if (gtk_notebook_get_current_page (GTK_NOTEBOOK(notebook)) == ARCHIVE_ASSISTANT_MENU)
	{
		if (gtk_widget_is_sensitive (initial_date))
		{
			...
			while (tmp_list)
			{
				...
				if (g_date_compare (init_gdate, gsb_data_transaction_get_date (transaction_number)) <= 0
					...
					list_transaction_to_archive = g_slist_append (list_transaction_to_archive, tmp_list->data);
		...
		else if (gtk_widget_is_sensitive (financial_year_button))
		{
			...
			if (!string && gsb_data_archive_get_from_fyear (fyear_number))
...
static GtkWidget *gsb_assistant_archive_page_success (void)
{
	...
	gtk_text_view_set_left_margin (GTK_TEXT_VIEW(congratulations_view), MARGIN_START);
	gtk_text_view_set_right_margin (GTK_TEXT_VIEW(congratulations_view), MARGIN_START);
	...
	gtk_text_buffer_create_tag (buffer, "indented", "left-margin", 2*MARGIN_START, NULL);
	...
	gtk_text_view_set_left_margin (GTK_TEXT_VIEW(failed_view), MARGIN_START);
	gtk_text_view_set_right_margin (GTK_TEXT_VIEW(failed_view), MARGIN_START);
															  ...
															  gint new_page)
{
	...
	gsb_assistant_change_button_next (assistant, "gtk-apply", GTK_RESPONSE_YES);
	...
	else if (gtk_widget_is_sensitive (financial_year_button))
	{
		...
		string = g_strdup_printf (_("Archive of financial year %s"), gsb_data_fyear_get_name (fyear));
	}
	else if (gtk_widget_is_sensitive (report_button))
	{
		gint report_number;
														 ...
														 gint new_page)
{
	...
	gsb_assistant_change_button_next (assistant, "gtk-close", GTK_RESPONSE_APPLY);
	...
	buffer = gtk_text_view_get_buffer (GTK_TEXT_VIEW (congratulations_view));
	gtk_text_buffer_get_iter_at_mark (buffer, &iter, gtk_text_buffer_get_mark (buffer, "status"));
...
GtkResponseType gsb_assistant_archive_run (gboolean origin)
{
	...
	if (origin)
	{
		...
		tmpstr = g_strdup_printf (_("There are a lot of transactions in your file (%d) "
									"and it is advised not to keep more than about %d transactions unarchived.\n"
