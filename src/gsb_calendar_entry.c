												...
												GtkWidget *entry)
{
	...
	gtk_entry_set_text (GTK_ENTRY (entry), gsb_format_date (day, month + 1, year));
											  ...
											  gpointer null)
{
	switch (ev->keyval)
	{
		...
		case GDK_KEY_KP_Page_Up :
			...
			if ((ev->state & GDK_CONTROL_MASK) != GDK_CONTROL_MASK)
				gsb_calendar_entry_step_date (entry, ONE_MONTH);
			else
				gsb_calendar_entry_step_date (entry, ONE_YEAR);
...
GDate *gsb_calendar_entry_get_date (GtkWidget *entry)
{
	...
	if (!entry || !GTK_IS_ENTRY (entry) || !gsb_date_check_entry (entry))
		return NULL;
